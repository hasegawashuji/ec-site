...
def addItem(request, slug):
    ...
    if order.exists():
        order = order[0]
        if order.items.filter(item__slug=item.slug).exists(): #Like句
...
class PaymentView(LoginRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        payment.stripe_charge_id = charge['id']    #'test_stripe_charge_id' #ダミーid
        ...
        order.ordered = True #注文完了フラグをたてる
